...
def verify_detailing():
    ...
    print(f"  s_detailing_max: {ana.s_detailing_max:.1f} mm")
    ...
    if "철근 간격검토" in total_text and "s_max = min( 2h, 250 )" in total_text:
        ...
        print("Debug: Report snippet around Spacing check:")
        # Print a small part of the report for debugging if not found
        start_idx = total_text.find("Reinforcement Check")
        if start_idx != -1:
            print(total_text[start_idx:start_idx+500])
